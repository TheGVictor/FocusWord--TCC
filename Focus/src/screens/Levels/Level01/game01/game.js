...
class WordSearchGame extends Component {
  ...
  handleCellClick = (x, y) => {
    ...
    if (palavraEncontrada) {
      // Verifique se a palavra já está selecionada.
      if (this.state.palavrasEncontradas.includes(palavraEncontrada)) {
        // A palavra já está selecionada, então desselecione-a.
        this.setState(
          (prevState) => ({
            palavrasEncontradas: prevState.palavrasEncontradas.filter(
              (palavra) => palavra !== palavraEncontrada
            ),
          }),
          () => {}
        );
      } else {
        // A palavra não está selecionada, então selecione-a.
        this.setState(
          (prevState) => ({
            palavrasEncontradas: [
